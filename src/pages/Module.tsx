...
const Module = () => {
  ...
  if (!stage) return <div className="p-10">Module not found. <Link to="/app" className="text-primary">Back</Link></div>;
    ...
    <div className="p-6 md:p-10 max-w-4xl">
      ...
      <div className="mt-6 mb-10">
        <div className="flex justify-between text-xs text-muted-foreground mb-2">
          <span>{done} of {stage.steps.length} complete</span><span>{pct}%</span>
        </div>
      ...
      <div className="space-y-3">
        {stage.steps.map((step, i) => {
          ...
          return (
            <div key={step.id} className={`rounded-2xl border bg-gradient-card p-5 transition-colors ${checked ? "border-primary/40" : "border-border"}`}>
              <div className="flex items-start gap-4">
                <button
                  onClick={() => toggleStep(stage.id, step.id)}
                  className={`mt-1 size-6 shrink-0 rounded-md border grid place-items-center transition-colors ${checked ? "bg-gradient-primary border-transparent text-primary-foreground" : "border-border hover:border-primary"}`}
